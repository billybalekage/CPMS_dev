...
exports.validateToken = (token) => {
  ...
  if (!/^\d{20}$/.test(token)) {
    return {
      isValid: false,
      error: "Le token doit contenir exactement 20 chiffres",
    };
  }
...
exports.validateMeterNumber = (meterNumber) => {
  ...
  if (meterNumber.trim().length < 3) {
    return {
      isValid: false,
      error: "Le numéro de compteur doit contenir au moins 3 caractères",
    };
  }
...
exports.isValidPaymentMethod = (paymentMethod) => {
  return ["cash", "card", "mobile_money", "bank_transfer"].includes(
    paymentMethod,
  );
};
